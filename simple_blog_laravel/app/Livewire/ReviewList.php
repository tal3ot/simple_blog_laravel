...
class ReviewList extends Component
{
    ...
    {
        $this->search = $search; //to prevent user from writing any thing on the url
    }
    ...
    //computed properties are properties that defined as a function and they give you the ability to build to easily cache them # called anotation 
    #[Computed()]
    public function reviews()
    {
        return Review::published()
            ...
            ->when($this->activeGenre, function ($query) {
                $query->withGenre($this->genre); //we can remove the word scope as it was scopeWithGenre
            })
            ->when($this->popular, function ($query) { //count likes first by the built in function then order  desc
                $query->withCount('likes')->orderBy('likes_count', "desc"); //if u like u can move it to review.php file and write in a scope function
            })
            ->where('title', 'like', "%{$this->search}%") //u can move this too
