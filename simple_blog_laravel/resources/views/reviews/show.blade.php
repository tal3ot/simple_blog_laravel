<x-app-layout :title="$review->title">
    <article class="col-span-4 md:col-span-3 mt-10 mx-auto py-5 w-full" style="max-width:700px">
        ...
        <div class="article-actions-bar my-6 flex text-sm items-center justify-between border-t border-b border-gray-100 py-4 px-2">
            <div class="flex items-center">
                <livewire:like-button :key="'likebutton-' . $review->id" :$review /> {{--the key to show the like button on all pages and be live, and review will go and update public function, when we don't use it, it says that $review must not be accessed before initializaion --}}
                {{--the key to show the like button on all pages and be live, and post will go and update public function and also this wire key is usually used to prevent some bugs or issues like the tag genres still while we change the pages or the like button doesn't change when we go to another page ...etc  --}}
            </div>
            <div>
                <div class="flex items-center">
                </div>
        ...
        <livewire:review-comments :key="'likecomment-' . $review->id" :$review /> {{--reivew will go and update public function, when we don't use it, it says that $review must not be accessed before initializaion --}}
        {{--the key to show the like button on all pages and be live, and post will go and update public function and also this wire key is usually used to prevent some bugs or issues like the tag genres still while we change the pages or the like button doesn't change when we go to another page ...etc  --}}
