...
<x-app-layout title="Home Page">
    ...
    <div class="mb-10"> {{-- if the images don't disappear write w-full --}}
        <div class="mb-16">
            <h2 class="mt-16 mb-5 text-3xl text-yellow font-bold">Featured Posts</h2>
            <div class="w-full">
                <div class="grid grid-cols-3 gap-10 w-full">
                    @foreach ($featuredReviews as $review)
                    <x-reviews.review-card :review="$review" class="rounded-xl bg-grey border border-orange-500 border-transparent hover:border-blue-800 group transition-colors duration-300" /> {{--from the props in the component/Reviews/review-card.blade--}}
                    @endforeach
                </div>
            </div>
        </div>
        ...
        <div class="w-full mb-5">
            <div class="grid grid-cols-3 gap-10 w-full"> {{--for formating the reviews well 3 side by side rather than 1 --}}
